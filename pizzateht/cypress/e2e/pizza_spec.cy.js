describe("Pizza-online -tilauksen testaus", () => {
  it("Tekee tilauksen ja tarkistaa loppusumman", () => {
    // Vieraillaan sivustolla
    cy.visit("https://tiko.jamk.fi/~imjar/fronttiper/esimteht/pizza_anim/");

    // -------------------------
    // 1) Lomakekentät
    // -------------------------

    cy.get('#nimi')
      .type("Matti Meikäläinen")
      .should("have.value", "Matti Meikäläinen");

    cy.get('#puhelin')
      .type("0401234567")
      .should("have.value", "0401234567");

    cy.get('#sposti')
      .type("matti@example.com")
      .should("have.value", "matti@example.com");

    // -------------------------
    // 2) Koko valintalistasta
    // -------------------------

  cy.get('#Normaali').check().should('be.checked');

    // -------------------------
    // 3) Pohjan valinta
    // -------------------------
  cy.get('#Ruis').check().should('be.checked');;

    // -------------------------
    // 4) Täytteet
    // -------------------------

  const taytteet = ["Kinkku", "Ananas", "Salami"]; // labelin tekstit

  taytteet.forEach(t => {
  cy.contains('label', t)
    .click()
    .find('input[type="checkbox"]') // valinnainen, jos haluat tarkistaa
    .should('be.checked');
  });

    // -------------------------
    // 5) Loppusumman tarkistus
    // -------------------------

    // Normaali koko = 10 €
    // 3 täytettä → 1 ilmainen + 2 × 0,50 = +1,00 
  const expectedTotal = "12.00 e";  // muoto kuten sivulla

  cy.contains('p', `Hinta: ${expectedTotal}`)
  .should('be.visible');
  });
});